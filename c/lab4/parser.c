...
// FIX: use a stack for GOSUB return addresses to support nested subroutines
#define GOSUB_STACK_SIZE 64
int retiStack[GOSUB_STACK_SIZE];
int retiTop = 0;  // stack pointer; 0 = empty
...
void statement() {
    ...
    switch (nextToken) {
        ...
        case GOSUB:
            lex();
            targetlineno = expression();
            // FIX: push return address onto stack instead of single int
            // so that nested subroutines work correctly
            if (retiTop >= GOSUB_STACK_SIZE) {
                printf("GOSUB stack overflow\n");
                exit(1);
            }
            retiStack[retiTop++] = linei; // save the index of the line we should return to
            // remember this is from a recursive call where
            // we are in the middle of a for loop that will execute
            // linei++ ... so we need to set it to the target index minus 1
            linei = findLine(targetlineno) - 1;
            ...
            break;
        case CLEAR:
            // FIX: free malloc'd strings before resetting lineindex to avoid memory leak
            for (int i = 0; i < lineindex; i++) {
                free(lines[i]);
                lines[i] = NULL;
            }
        ...
        case LIST:
            ...
            break;
        case RETURN:
            // FIX: pop return address from stack instead of using single int
            if (retiTop <= 0) {
                printf("RETURN without GOSUB\n");
                exit(1);
            }
            linei = retiStack[--retiTop];
            lex();
            break;
        case RUN:
            sort();
...
void expr_list() {
    // FIX: print string BEFORE calling lex() so lexeme still holds the string value
    if (nextToken == STRING) {
        printf("%s\t", lexeme); // print BEFORE lex() overwrites lexeme
...
// FIX: correctly apply unary + and - instead of silently ignoring the sign
int expression() {
    int sign = 1;
    if (nextToken == SUB_OP) {
        sign = -1;
        lex(); // move past the leading -
    } else if (nextToken == ADD_OP) {
        lex(); // move past the leading + if it was there otherwise, the current nextToken is part of the term so no need to call lex()
    }
    ...
    while (nextToken == ADD_OP || nextToken == SUB_OP) {
        int op = nextToken; // save op BEFORE lex() overwrites nextToken
